...
void write_parquet_file(const arrow::Table &table) {
    ...
    PARQUET_ASSIGN_OR_THROW(
            outfile,
            arrow::io::FileOutputStream::Open("/data/test/res_test2.parq"));
    ...
    PARQUET_THROW_NOT_OK(
            parquet::arrow::WriteTable(table, arrow::default_memory_pool(), outfile, 100000,props));

    cout<<"写入完毕"<<endl;
}

// #2: Fully read in the file
std::shared_ptr<arrow::Table> read_whole_file() {
    std::cout << "Reading parquet-arrow-example.parquet at once" << std::endl;
    std::shared_ptr<arrow::io::ReadableFile> infile;
    PARQUET_ASSIGN_OR_THROW(
            infile,
            arrow::io::ReadableFile::Open("/data/test/res.parq",
...
//    arrow::PrettyPrint(*table, 4, &std::cout);
    std::cout << std::endl;
    return table;
}
...
//    write_parquet_file(*table);
    Timeit ti("读取总文件");
    auto table = read_whole_file();
    ti.report();
    Timeit ti2("写parquet文件"+to_string(table->num_rows()));
    write_parquet_file(*table);
    ti2.report();
